...
class PositionalEncoding(nn.Module):
    def __init__(self, hidden_size, max_len=512):
        ...
        p_e = torch.zeros(max_len, hidden_size, requires_grad=False)
        factor = torch.arange(0, hidden_size, 2)
        factor = factor *-(math.log(10000.0) / hidden_size)
        factor = torch.exp(factor)
        
        pos = torch.arange(0, max_len).unsqueeze(1)
        
        p_e[:, 0::2] = torch.sin(pos * factor)
        p_e[:, 1::2] = torch.cos(pos * factor)
        pe = p_e.unsqueeze(0)
...
def look_ahead_mask(size):
    ...
    an_sh = (1, size, size)
    look_ahead_mask = np.triu(np.ones(an_sh), k=1).astype('uint8')
...
class FeedForward(nn.Module):
    def __init__(self, hidden_size, ff_hidden_size, dropout=0.1):
        ...
        self.pre_l = nn.Linear(hidden_size, ff_hidden_size)
        self.post_l = nn.Linear(ff_hidden_size, hidden_size)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        return self.post_l(self.dropout(F.relu(self.pre_l(x))))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # from hidden_size => n_heads x head_hidden_size

        query, key, value = \
            [i(x).view(batch_size, -1, self.n_heads, self.head_hidden_size).transpose(1, 2)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        s_l = self.sublayers[1](x, self.feed_forward)
        return s_l
...
def train(source_strings, target_strings):
    ...
    model_config = {
        ...
        'max_src_seq_length': max(train_df['en'].aggregate(len)) + 2, 
...
class SmoothCrossEntropyLoss(_WeightedLoss):
    ...
    def k_one_hot(self, targets:torch.Tensor, n_classes:int, smoothing=0.1):
        with torch.no_grad():
...
def gs(smoothness):
    ...
    datasets = load_datasets('./TRANSLIT', ['train_small', 'dev_small'])

    train_en = datasets['train_small']['en']
    train_ru = datasets['train_small']['ru']
    
    learnable_params = train(train_en, train_ru, smoothness)
    
    test_en = datasets['dev_small']['en']
    test_ru = datasets['dev_small']['ru']

    preds = classify(test_en, learnable_params)
    mv = compute_metrics(np.squeeze(preds), test_ru, ['acc@1', 'mean_ld@1'])
